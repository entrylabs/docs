...
이 함수의 호출 받은 서버측 api는 formData에 포함된 모델의 model.json 과 model.weights.bin(TFJS를 사용하는 타입인경우) 를 저장소에 저장하는 로직을 구현해야 합니다.
...
| 파라미터 | 타입   | 선택적 | 기본값 | 설명                               |
| -------- | ------ | ------ | ------ | ---------------------------------- |
| _id      | string |        |        | db에서 사용하는 모델의 id값입니다. |
| data     | string |        |        | ...등 모델 학습에 상요된 데이터 값 |
...
| 파라미터  | 타입   | 선택적 | 기본값 | 설명                                 |
| --------- | ------ | ------ | ------ | ------------------------------------ |
| projectId | string |        |        | data를 포함하는 프로젝트의 id입니다. |
| classId   | string |        |        | 작성중                               |
| saveData  | any    |        |        | 작성중                               |
