...
app.route('getModelSaveUrl').get(save);
...
// 서버측 예시코드
app.route('saveModelFile에서 호출하는 api경로').post(saveModelFile);

const saveModelFile = async(req, res) => {
...
![ml_popup_deleteModel1](/images/entry-ml/ml_popup_deleteModel2.png)
...
![ml_popup_deleteModel1](/images/entry-ml/ml_popup_copyModel1.png)
...
app.route('copyModel에서 호출하는 api경로').delete(deleteModel)

const copyModel = async (req, res) => {
  const { modelId } = req.params;
  const model = await db.model.find({ id: modelId });
  
  // 조회한 모델의 내용을 복사하고 유니크한 값 제거
