...
const API_PREFIX = '/rest/learning/';
const CDN_PREFIX = '/uploads';

class EntryApi {
    getModelSaveUrl = (modelId) => `${API_PREFIX}${modelId}`;
    getModelDownloadUrl(basePath) {
        const time = new Date().getTime();
        return `${CDN_PREFIX}/${basePath}/model.json?${time}`;
    }
    ...
    async downloadModel(modelId) {
        Utils.download(`/rest/learning/download/${modelId}`, { method: 'POST' });
    }
...
tfJS가 사용된 모델학습이 완료되었을 때 사용합니다.

모델을 저장할 api url을 반환합니다.

tensorflow JS를 사용하는 타입에서 사용합니다.
...
getModelSaveUrl에서 저장한 model.json, model.weight.json 파일의 상위 디렉토리 주소.
...
'학습할 모델 선택하기' > '모델 학습하기' 에서 선택하여 학습할 모델 데이터를 로드합니다.
...
| 파라미터                                               | 타입                                                         | 선택적 | 기본값 | 설명                                      |
| ------------------------------------------------------ | ------------------------------------------------------------ | ------ | ------ | ----------------------------------------- |
| {<br /> url?: string; <br /> modelId?: string; <br />} | { <br />url: getModelDownloadUrl 경로<br />modelId: 로드할 model의 id값 |        |        | 가져오는 방식<br />1. url<br />2. modelId |
