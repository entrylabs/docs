...
```bash
npm install
# install 후 자동으로 실행되는 postinstall 스크립트에 아래 내용이 포함되어있으니 생략해도 됩니다.
...
class Module extends BaseModule {
    ...
    // 하드웨어 연결 해제 시 호출됩니다.
    // 만약 이 함수가 선언되어있다면, 명시적으로 connect.close() 해주셔야 합니다.
    // 선언되어 있지 않다면 엔트리 하드웨어는 connect.close() 를 수행합니다.
    // @see https://github.com/entrylabs/entry-hw/blob/e69bf9b202fa3caf608ab3913afc86d304cd8333/app/src/main/mainRouter.js#L356-L360
    disconnect(connect) {
        var self = this;
        connect.close();
        if (this.sp) {
